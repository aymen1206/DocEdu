...
<div class="letter-form-wrapper">
    <h2>نموذج إنشاء خطاب إستدعاء ولي امر </h2>

    <form method="post" action="{{ route('edu-facility.appreciation.pdf') }}" id="pdfForm">
        @csrf
    ...
    <form method="post" action="{{ route('edu-facility.appreciation.word') }}" style="margin-top: 16px;" onsubmit="document.getElementById('word_title').value=document.getElementById('letter_title').value;document.getElementById('word_content').value=document.getElementById('letter_content').value;">
        @csrf
